...
/**
 * The Class StaticDocument attempts to display the document of a given folder,
 * so if the user selects the CATALOG, they see the entire catalog, but if they
 * select COM S, they see just the COM S part of the catalog. Some effort has
 * been made so that at least the Courses are styled appropriately.
 */
//TODO - make the headings style-able
...
public class StaticDocument extends DefaultStyledDocument {
	...
	/**
	 * Instantiates a new static document.
	 * 
	 * @param root
	 *            the root
	 */
	...
	/**
	 * Gets the v specs from element.
	 * 
	 * @param e
	 *            the e
	 * @return the v specs from element
	 */
	private ArrayList<ElementSpec> getVSpecsFromElement(Element e) {
		ArrayList<ElementSpec> vSpecs = new ArrayList<ElementSpec>();
		String tagName = e.getName();
		if (Main.debug3) {
			System.out.println("getting vspecs from element with tagname: "
					+ tagName);
		}
		...
		if (tagName.equals("HEADER")) {
			...
			while (scan.hasNextLine()) {
				...
				textAttrs.addAttribute(VersionDocument.PARAM_WIDTH,
						new Integer(line.length()));
				ElementSpec start = new ElementSpec(textAttrs,
						ElementSpec.StartTagType);
				vSpecs.add(start);
				ElementSpec parContent = new ElementSpec(
						new SimpleAttributeSet(), ElementSpec.ContentType,
						line.toCharArray(), 0, line.length());
				vSpecs.add(parContent);
				ElementSpec end = new ElementSpec(textAttrs,
			...
			// since this facade will go out of scope once the ArrayList is
			// returned
			...
			for (javax.swing.text.Element textElement : list) {
				...
				String content = "";
				if (!(elementName.equals("paragraph") || elementName
						.equals("content"))) {
					try {
						content = doc.getText(
								textElement.getStartOffset(),
								textElement.getEndOffset()
										- textElement.getStartOffset());
